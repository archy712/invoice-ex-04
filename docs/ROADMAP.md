...
- **Task 008: 견적서 유효성 검증 및 404 처리 구현 (F011)** ✅ - 완료
  - ✅ 존재하지 않는/잘못된 `notionPageId` 접근 시 Notion API 에러 캐치 → `notFound()` 호출
  - ✅ 권한 없는 페이지·잘못된 형식 ID 케이스 분기 처리(`InvoiceNotFoundError` + `ObjectNotFound`/`RestrictedResource` 코드 매핑, 형식 검증은 API 호출 전 정규식으로 선제 처리)
  - ✅ Zod 파싱 실패 및 API 키 오류 등 설정성 에러는 404가 아닌 기존 `error.tsx`로 계속 흡수(Task 007 동작 유지, 회귀 없음)
  - **## 테스트 체크리스트 (Playwright MCP)** — 모든 시나리오 Pass 완료 (성공 기준 5):
    ...
    - ✅ 에러: 형식 자체가 잘못된 ID(빈 문자열·비정상 길이·특수문자)에서도 Notion API 호출 없이 404로 안전하게 폴백되는지 검증
    - ✅ 회귀: API 키 오류(Unauthorized) 등 설정성 에러는 404가 아닌 `error.tsx`로 흡수되는지 검증
    - ✅ 회귀: Zod 파싱 실패 케이스에서 여전히 `error.tsx`로 안전하게 폴백되는지 검증(부분 렌더/크래시 없음)
  - 상세: `tasks/008-invoice-validation-404.md`
