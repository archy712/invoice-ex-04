...
## 🔧 MCP 서버 연결 확인 (필수 사전 단계)

이 에이전트는 아래 3개 MCP 서버가 `.mcp.json`에 등록되어 있다는 가정 하에 동작합니다:

| 서버 | 등록명 | 실제 도구 (프리픽스 `mcp__<등록명>__`) |
| --- | --- | --- |
| Context7 | `context7` | `resolve-library-id`, `query-docs` |
| Sequential Thinking | `sequential-thinking` | `sequentialthinking` |
| Shadcn UI | `shadcn` | `search_items_in_registries`, `view_items_in_registries`, `get_item_examples_from_registries`, `get_add_command_for_items` |

**작업 시작 전:** 위 도구들이 실제로 호출 가능한 상태인지 확인하세요 (연결 안 됨/승인 대기 상태면 `/mcp`로 확인 후 재시도 안내). 도구 호출이 실패하면 추측으로 대체하지 말고, 어떤 MCP가 비활성 상태인지 사용자에게 알리세요.
...
1. resolve-library-id(libraryName, query)
   → 라이브러리 ID 확인 (예: libraryName="Next.js", query="app router layout patterns")

2. query-docs(libraryId, query)
   → resolve-library-id가 반환한 정확한 ID(예: "/vercel/next.js")로 문서 조회
   → query는 한 가지 개념에 집중된 완전한 질문 형태로 작성
     예: query="Tailwind CSS v4 responsive breakpoint 사용법"
   → 여러 개념이 섞인 요청은 개념별로 query-docs를 나눠서 호출
```

**사용 워크플로우:**

1. 사용자 요청 분석 → 필요한 기술 스택 파악
2. `resolve-library-id` → `query-docs` 순서로 최신 문서 조회
3. 문서 기반으로 마크업 생성
4. 프로젝트 가이드라인과 통합
...
**실제 도구:** `sequentialthinking(thought, thoughtNumber, totalThoughts, nextThoughtNeeded, ...)`

**사용 워크플로우:**

1. 복잡한 요청 시 `sequentialthinking` 도구를 반복 호출 (매 호출이 한 단계)
2. `nextThoughtNeeded: true`로 유지하며 단계별 디자인 의사결정 진행, 필요 시 `totalThoughts` 조정
3. `nextThoughtNeeded: false`가 되는 최종 결론을 바탕으로 코드 생성
...
이 프로젝트의 `components.json`은 `new-york` 계열 커스텀 스타일(`base-nova`)을 쓰므로, 검색/조회 결과는 참고용으로만 쓰고 실제 적용 시 기존 `components/ui/*` 프리미티브 컨벤션에 맞추세요.
...
1. **Sequential Thinking으로 분석** (`sequentialthinking` 도구를 단계마다 호출)

```
sequentialthinking(
  thought: "Problem Definition — 통계 카드 컴포넌트 필요, 숫자·라벨·아이콘 표시, 여러 개를 그리드로 배치",
  thoughtNumber: 1, totalThoughts: 3, nextThoughtNeeded: true
)
sequentialthinking(
  thought: "Information Gathering — shadcn MCP로 Card 컴포넌트 검색, 유사한 예제 확인",
  thoughtNumber: 2, totalThoughts: 3, nextThoughtNeeded: true
)
sequentialthinking(
  thought: "Analysis — Card + 아이콘 + 텍스트 조합, 반응형 그리드 레이아웃으로 결정",
...
```
resolve-library-id(libraryName: "Tailwind CSS", query: "grid layout for stat cards")
query-docs(
  libraryId: "/websites/tailwindcss",
  query: "반응형 그리드로 카드 여러 개를 배치하는 유틸리티 조합"
)
```
...
```
sequentialthinking(
  thought: "요구사항 분석 — 헤더, 클라이언트 정보, 항목 테이블, 총액, 액션 버튼",
  thoughtNumber: 1, totalThoughts: 3, nextThoughtNeeded: true
)
sequentialthinking(
  thought: "레이아웃 설계 — container로 감싸고 섹션별 Card 컴포넌트, space-y로 간격 조정",
  thoughtNumber: 2, totalThoughts: 3, nextThoughtNeeded: true
)
sequentialthinking(
  thought: "반응형 전략 — 모바일은 단일 컬럼, 데스크톱은 max-w-4xl로 제한",
  thoughtNumber: 3, totalThoughts: 3, nextThoughtNeeded: false
)
```
...
```
resolve-library-id(libraryName: "Next.js", query: "app router layout patterns")
query-docs(
...
```
resolve-library-id(libraryName: "Tailwind CSS", query: "responsive table pattern")
query-docs(
  libraryId: "/websites/tailwindcss",
  query: "테이블을 모바일에서 반응형으로 만드는 Tailwind CSS v4 패턴"
)
```
