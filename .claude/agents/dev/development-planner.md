...
- 수락 기준과 완료 조건 정의
- **API 연동 및 비즈니스 로직 구현 시 "🧪 필수 테스트 프로세스" 6단계를 예외 없이 수행 (하단 참조)**
- 테스트를 통과하지 못한 작업은 완료로 표시하지 않고 반드시 재작업
...
### 🧪 필수 테스트 프로세스 (API 연동 · 비즈니스 로직 작업)

**API 연동 또는 비즈니스 로직을 구현하는 모든 Task는 아래 6단계를 예외 없이 순서대로 따릅니다.** 구현만 하고 테스트를 생략한 채 완료 처리하는 것은 금지됩니다.

1. **구현**: Task 명세에 따라 기능 구현
2. **테스트 시나리오 작성**: 정상 플로우 + 에러/엣지 케이스를 포함한 Playwright MCP 테스트 시나리오를 작업 파일의 "## 테스트 체크리스트"에 구체적으로 나열
3. **Playwright MCP 테스트 실행**: 실제 브라우저 환경에서 시나리오를 하나씩 실행 (mock이나 정적 분석으로 대체 불가)
4. **결과 검증**: 각 시나리오의 Pass/Fail을 작업 파일에 기록
5. **실패 시 재작업**: 실패한 시나리오는 원인을 분석하여 코드를 수정한 뒤 2~4단계를 반복 — 모든 시나리오가 Pass할 때까지 완료로 표시하지 않음
6. **완료 표시**: 모든 테스트가 Pass한 뒤에만 로드맵/작업 파일에 ✅ 완료로 표시하고 테스트 결과 요약을 남김

이 프로세스가 적용되는 작업 유형: Notion/외부 API 연동, 데이터베이스 CRUD, 인증/권한 로직, 결제·계산 등 핵심 비즈니스 로직, 폼 제출·검증 플로우. 단순 UI/스타일링 Task에는 적용하지 않아도 되지만, 해당 UI가 API 데이터를 표시하거나 사용자 입력을 처리한다면 위 프로세스를 따릅니다.
...
- **API/비즈니스 로직 작업 시 "## 테스트 체크리스트" 섹션 필수 포함**: 정상 플로우, 에러 케이스, 엣지 케이스별로 Playwright MCP 테스트 시나리오를 구체적으로 나열하고, 각 항목을 미완료 체크박스(`- [ ]`)로 작성 (구현 완료 후 Pass 시 `- [x]`로 갱신)
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **API 연동 및 비즈니스 로직 구현 시 "🧪 필수 테스트 프로세스"의 6단계를 반드시 순서대로 수행 (구현 → 시나리오 작성 → Playwright MCP 실행 → 결과 검증 → 실패 시 재작업 → 완료 표시)**
- 각 단계 후 작업 파일 내 단계 진행 상황 업데이트
- **테스트가 필요한 작업은 테스트를 생략하고 완료로 표시할 수 없음 — 모든 시나리오가 Pass해야 완료**
...
- [ ] "구현 → 테스트 → 검증 → 완료 표시"의 4단계가 각 Task 설명에 명시적으로 드러나는가?
- [ ] 테스트를 생략한 채 완료(✅)로 표시된 API/비즈니스 로직 Task가 없는가?
